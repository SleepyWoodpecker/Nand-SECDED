...
static uint16_t eal_bit_sequence_parity(uint32_t input);
static uint16_t eal_get_message_parity(const uint32_t raw_data[]);
static ParityBlock_t eal_split_parity_bits(const uint16_t parity_bits, const int set_number);
static uint16_t eal_extract_block_parity_sequence(const uint8_t parity_sequence[], const int parity_sequence_idx, const int set_number);
...
uint16_t eal_encode_256(const uint32_t raw_data[]) {
    ...
    for (int i = 0; i < INDIVIDUAL_PARITY_BITS; ++i) {
        ...
        sequence_parity ^= eal_bit_sequence_parity(raw_data[j] & parity_generator_idxs[i][j]);
        }
        parity_bits <<= 1;
...
uint16_t eal_encode_overall_parity(const uint32_t raw_data[], const uint16_t parity_bits) {
    uint16_t current_parity_sum = eal_get_message_parity(raw_data);
    uint16_t sequence_parity_bits = parity_bits & 0b111111111;
    current_parity_sum ^= eal_bit_sequence_parity(sequence_parity_bits);
...
void eal_check_256(const uint32_t raw_data[], const uint16_t parity_bits, DecodeResponse_t *decode_response) {
    ...
    for (int i = 0; i < TOTAL_NUM_PARITY_BITS ; ++i) {
        uint16_t local_bit_sum = eal_bit_sequence_parity(parity_bits & decode_matrix[i][0]);
        for (int j = 1; j < NUM_COLS_IN_DECODE_MATRIX; ++j) {
        local_bit_sum ^= eal_bit_sequence_parity(raw_data[j - 1] & decode_matrix[i][j]);
        }
    ...
    decode_response->response_flags = (
        BIT_CORRECTED | 
        ((overall_parity_bit != eal_encode_overall_parity(raw_data, parity_bits)) ? OVERALL_PARITY_INVALID : 0)
    );
...
void eal_encode_page(uint8_t flash_block[]) {
    ...
    uint8_t *parity_bit_sequences = flash_block + SECDED_REGION_START_OFFSET_IDX;
    uint32_t *r_raw_data = (uint32_t *)flash_block;
    size_t parity_bit_sequences_idx = 0;

    for (int i = 0; i < NUM_BLOCKS_IN_PAGE; ++i) {
        uint16_t parity_seq = eal_encode_256(r_raw_data);
        uint16_t overall_parity_seq = eal_encode_overall_parity(r_raw_data, parity_seq);

        uint16_t overall_parity = (overall_parity_seq << INDIVIDUAL_PARITY_BITS) | parity_seq; 
        
        ParityBlock_t parity_blocks = eal_split_parity_bits(overall_parity, i);
...
bool eal_check_page(uint8_t raw_data[], uint8_t parity_bit_sequences[]) {
    ...
    for (int i = 0; i < NUM_BLOCKS_IN_PAGE; ++i) {
        ...
        uint16_t parity_bits = eal_extract_block_parity_sequence(parity_bit_sequences, parity_bit_sequence_idx, i);
        eal_check_256(r_raw_data, parity_bits, &decode_response);

        if (!eal_resolve_decode(r_raw_data, &decode_response)) {
        return false;
        }
...
static uint16_t eal_get_message_parity(const uint32_t raw_data[]) {
    ...
    for (int i = 0; i < NUM_32_BIT_COLS_IN_BLOCK; ++i) {
        current_parity_sum ^= eal_bit_sequence_parity(raw_data[i]);
    }
    return current_parity_sum;
}
